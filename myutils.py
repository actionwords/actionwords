...
dataprep = '/nfs/projects/firstwords/data/standard'
...
class batch_gen(keras.utils.Sequence):
    def __init__(self, seqdata, firstwords, tt, config, training=True):
    ...
    def make_batch(self, batchfids):
        if self.config['batch_maker'] == 'datsonly':
            return self.divideseqs_datsonly(batchfids, self.seqdata, self.comvocabsize, self.tt)
        elif self.config['batch_maker'] == 'seqsonly':
    ...
    def divideseqs(self, batchfids, seqdata, comvocabsize, tt):
        ...
        for fid in batchfids:
            ...
            if not self.training:
                ...
                comout = keras.utils.to_categorical(self.firstwords[fid],
                                                    num_classes=comvocabsize)
        ...
        if not self.training:
            ...
            return [[smlseqs], comouts]

    def divideseqs_datsonly(self, batchfids, seqdata, comvocabsize, tt):
        import keras.utils
