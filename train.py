...
if __name__ == '__main__':
    ...
    print('seqdata[config]: ', seqdata['config'])
    print('seqdata.keys(): ', seqdata.keys())
    print('config: ', config)
    try:
        config['comlen'] = 1
    ...
    gen = batch_gen(seqdata, firstwords, 'train', config)
    ...
    valgen = batch_gen(seqdata, firstwords, 'test', config)

    callbacks = [ checkpoint, savehist ]

    try:
        history = model.fit_generator(gen, steps_per_epoch=steps, epochs=epochs, verbose=1, max_queue_size=8, workers=1, use_multiprocessing=True, callbacks=callbacks, validation_data=valgen, validation_steps=valsteps)
    except Exception as ex:
        print(ex)
        traceback.print_exc(file=sys.stdout)
